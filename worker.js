/*
 * BSE GEMINI RESULT ANALYZER - WORKER BACKEND
 * 
 * GitHub Repo: bse-gemini-resultanalyzer
 * KV Binding: BSE_GEMINI_DATA (ID: 6c86cb7981334c8ba2b1ed48d73e3d2a)
...
async function analyzeFinancialPdf(pdfUrl, env) {
  ...
  try {
    ...
    // Convert binary PDF buffer to base64
    let binary = "";
...
async function fetchXML(url) {
  const response = await fetch(url, {
    method: "GET",
    headers: {
      "User-Agent": "Mozilla/5.0 (compatible; BSE-Gemini-Analyzer/1.0)",
...
async function monitorFeeds(env) {
  ...
  for (const item of newItems) {
    ...
    // Trigger AI analysis on ALL financial result PDFs, or send standard alert for whitelisted stocks
